...
public actor NISession {
  ...
  init(identifier: String, timeout: TimeInterval) async throws {
    viSession = try await Self.rawSession(withIdentifier: identifier, timeout: timeout)
...
extension NISession {
  ...
  static func rawSession(
  ...
  ) async throws -> ViSession {
    let instrumentManagerSession = try await InstrumentManager.niInstrumentManager.session
    ...
    try await NIVISAXPCCommunicator.shared
      .assertingServiceConnected { (service, status) -> ViStatus in
        ...
        (status, session) = await service.open(session: instrumentManagerSession,
                                               resourceName: identifier,
                                               mode: ViAccessMode(VI_NULL),
                                               timeout: ViUInt32(1_000 * timeout))
...
extension NISession: Session {
  ...
  public func reconnect(timeout: TimeInterval) async throws {
    try await close()
    viSession = try await Self.rawSession(withIdentifier: identifier, timeout: timeout)
  }
}
